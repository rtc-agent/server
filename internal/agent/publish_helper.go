...
// setSessionStatus updates the session's status and publishes a session.updated
// event. Used independently of the lifecycle batch publish (e.g., from other
// code paths that need to change session status without a turn event).
//
//nolint:unused // Reserved for future use cases
func (h *helpers) setSessionStatus(ctx context.Context, sessionID uuid.UUID, status protocol.SessionStatus, action string) {
	if err := h.deps.SessionRepo.UpdateStatus(ctx, sessionID, status); err != nil {
		h.logIfEnabled(ctx, "setSessionStatus.update_failed", map[string]any{
			"session_id": sessionID.String(),
	...
	h.logIfEnabled(ctx, "setSessionStatus.done", map[string]any{
		"session_id": sessionID.String(),
		"status":     string(status),
		"action":     action,
	})

	// Publish session.updated event so the frontend sees the status change.
	if h.deps.UpdatePublisher == nil {
		return
	}
	session, err := h.deps.SessionRepo.GetByID(ctx, sessionID)
	if err != nil {
		h.logIfEnabled(ctx, "setSessionStatus.load_session_failed", map[string]any{
			"session_id": sessionID.String(),
			"action":     action,
			"error":      err.Error(),
		})
	...
	if len(updates) > 0 {
		if _, err := h.deps.UpdatePublisher.Publish(ctx, updates...); err != nil {
			h.logIfEnabled(ctx, "setSessionStatus.publish_failed", map[string]any{
				"session_id": sessionID.String(),
				"action":     action,
				"error":      err.Error(),
			})
